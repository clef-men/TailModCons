...
(* From perennial/iris-inv-hierarchy *)

(** XXX: TODO, upsteam, although this proof is a mess *)
Lemma auth_frac_update {SI} {A: ucmraT SI} q (a b b': A) :
...
Lemma auth_frac_update_alloc {SI} {A: ucmraT SI} (q: Qp) (a b': A):
  (a, ε) ~l~> (a,b') → (●{q} a ~~> ●{q} a ⋅ ◯ b').
Proof. intros. rewrite -{1}(right_id _ _ (●{q} a)). by eapply auth_frac_update in H. Qed.

Lemma auth_frac_update_core_id {SI} {A: ucmraT SI} q (a b: A) `{!CoreId b} :
  b ≼ a → ●{q} a ~~> ●{q} a ⋅ ◯ b.
Proof.
