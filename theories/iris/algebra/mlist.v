...
From iris.algebra Require Import auth gmap auth_frac.
...
Section cmra_mlist.
  ...
  Context (A: Type) `{EqDecision A} {SI: indexT}.
  ...
  Canonical Structure mlistC : ofeT SI := discreteO SI mlist.
  ...
  Instance anti_symm_prefix_of : AntiSymm eq (@list.prefix A).
  Proof.
  ...
  Proof.
    ...
    constructor.
    apply (anti_symm list.prefix); auto.
  Qed.
  ...
  Canonical Structure mlistR := discreteR SI mlist mlist_ra_mixin.

  Global Instance mlistR_cmra_discrete : CmraDiscrete mlistR.
  Proof. apply discrete_cmra_discrete. Qed.

  Definition mlist_ucmra_mixin : UcmraMixin SI mlistR.
  Proof.
  ...
  Canonical Structure mlistUR :=
    UcmraT SI mlist mlist_ucmra_mixin.
...
Definition fmlistUR {SI} (A: Type) {Heq: EqDecision A} := (@authUR SI (mlistUR A)).
Class fmlistG {SI} (A: Type) {Heq: EqDecision A} (Σ: gFunctors SI) :=
  { fmlist_inG :> inG Σ (fmlistUR A) }.
Definition fmlistΣ {SI} (A: Type) {Heq: EqDecision A} : gFunctors SI :=
  #[GFunctor (fmlistUR A)].

Instance subG_fmlistΣ {SI} (A: Type) {Heq: EqDecision A} {Σ : gFunctors SI} :
  subG (fmlistΣ A) Σ → (fmlistG (SI:=SI) A) Σ.
Proof. solve_inG. Qed.

Section fmlist_props.
  Context `{Σ : gFunctors SI}.
  Context `{@fmlistG SI A Heq Σ}.
...
Instance inj_MList_equiv : Inj eq (@equiv _ (mlist_equiv _)) (@MList A).
...
Lemma fmlist_update_snoc x γ l:
  fmlist γ 1 l ==∗ fmlist γ 1 (l ++ [x]) ∗ fmlist_lb γ (l ++ [x]).
Proof.
  iIntros "Hm". iMod (fmlist_update with "[$]") as "($&$)"; last done.
  by exists [x].
Qed.
