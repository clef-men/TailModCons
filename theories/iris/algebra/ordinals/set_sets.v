...
Require Export iris.algebra.ordinals.set_model.
...
- apply zf_extensionality; split. eauto using spec_subs. intros x J. apply zf_specification. eauto.
Qed.
...
- apply zf_union. apply empty_el in J as [A H]. exists A. split. exact (I A H). assumption.
- assumption.
Qed.
...
- apply zf_union. exists A. split. apply I. apply single_el. reflexivity.
Qed.
...
- apply zf_specification. split. apply zf_union. exists A. split. apply I. apply single_el. eauto.
...
exists A. split. apply I. apply interI.
...
- subst b. apply single_el in J. subst a. cut (A=B). intros H. subst A. assumption.
  apply opair_intuni. apply J'. apply interI.
+ intros a H. apply zf_pair in H as [H|H]; subst a. by apply single_el. apply zf_pair. eauto.
+ apply empty_el. exists (singleton A). apply upair1.
...
Proof.
split; intros I.
- split. rewrite <- (pi1_cor A B), <- (pi1_cor A' B'). by rewrite I.
...
Proof.
destruct (classic (A = ∅)). rewrite H. apply product_empty1. apply empty_el in H as [a H].
...
+ apply single_el in I. apply zf_replacement. exists a. split. assumption. by rewrite -> repl_empty.
Qed.
...
+ apply zf_replacement. exists b. split. apply B'. assumption.
+ apply zf_replacement. exists a. split. apply A'. reflexivity.
Qed.
...
Proof.
intros I. split. eauto using product_pi1. eauto using product_pi2.
Qed.
