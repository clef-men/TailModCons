...
From iris.algebra.ordinals Require Export set_model.
From iris.algebra.ordinals Require Import set_sets set_functions.
...
- assert (OA: ordinal a) by eauto.
  assert (J: wfounded (elorder a) a). now destruct OA as [_[]].
  specialize (J (binter_set a M) (binter_subs1 a M) H). destruct J as [x [H1 H2]].
  exists x. split. now apply binterE in H1 as []. apply binterE in H1 as [H1 H1'].
...
Proof.
  intros H. eapply two_cycles. split. apply H. apply succ_set_el.
Qed.
...
Proof.
  ...
  - destruct H2 as [H | H]; [easy | ].
    ...
    + (* b is the largest ordinal < a *)
      ...
      {
        intros c Hel. assert (Hordc : ordinal c). { eapply ordinal_el. apply Hord. eauto. }
...
Proof.
  ...
  - exfalso. eapply succ_set_smallest. now apply Hel. now apply Hl.
Qed.
