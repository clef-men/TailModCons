...
(* This is a backport of auth_map.v by Tej Chajed from the Perennial project. *)

Set Default Goal Selector "!".
Set Default Proof Using "Type".

Definition mapUR (SI: indexT) (K V: Type) `{Countable K}: ucmraT SI :=
  gmapUR K (csumR SI (prodR (fracR SI) (agreeR (leibnizO SI V)))
                  (agreeR (leibnizO SI V))).

Class mapG {SI} Σ K V `{Countable K} :=
  { map_inG :> inG Σ (authUR (mapUR SI K V)); }.

Definition mapΣ {SI} K V `{Countable K} :=
  #[GFunctor (authR (mapUR SI K V))].

Instance subG_mapG (SI : indexT) K V `{Countable K} :
  subG (mapΣ (SI:=SI) K V) Σ → mapG (SI:=SI) Σ K V.
Proof. solve_inG. Qed.

Section auth_map.
  Context {K V: Type}  `{Countable0: Countable K}.
  Implicit Types (γ:gname) (k:K) (q:Qp) (v:V) (m: gmap K V).
  Context {SI} {Σ : gFunctors SI} `{!mapG Σ K V}.

  Definition to_mapUR : gmap K (V*bool) → mapUR SI K V :=
    fmap (λ '(v, ro), if (ro:bool) then Cinr (to_agree (v : leibnizO SI V))
                      else Cinl (1%Qp, to_agree (v : leibnizO SI V))).
  ...
  Lemma to_mapUR_insert_inl k v (m: gmap K (V*bool)) :
    to_mapUR (<[k:=(v,false)]> m) = <[k:=Cinl (1%Qp, to_agree (v:leibnizO SI V))]> (to_mapUR m).
  Proof. rewrite /to_mapUR fmap_insert //. Qed.

  Lemma to_mapUR_insert_inr k v (m: gmap K (V*bool)) :
    to_mapUR (<[k:=(v,true)]> m) = <[k:=Cinr (to_agree (v:leibnizO SI V))]> (to_mapUR m).
  Proof. rewrite /to_mapUR fmap_insert //. Qed.
  ...
  Definition ptsto_def γ k mq v :=
    own γ (◯ {[ k := match mq with
                     | Some q => Cinl (q, to_agree (v : leibnizO SI V))
                     | None => Cinr (to_agree (v : leibnizO SI V))
                     end ]}).
  ...
  Lemma Cinl_valid (A B:cmraT SI) (x:A) :
    ✓ @Cinl A B x → ✓ x.
  Proof. auto. Qed.

  Lemma Cinr_valid (A B:cmraT SI) (x:B) :
    ✓ @Cinr A B x → ✓ x.
  Proof. auto. Qed.

  Lemma Cinl_Cinr_op (A B:cmraT SI) x y :
    @Cinl A B x ⋅ @Cinr A B y = CsumBot.
  Proof. reflexivity. Qed.

  Lemma Cinr_Cinl_op (A B:cmraT SI) x y :
  ...
  Lemma ptsto_agree_frac_value γ k mq1 mq2 v1 v2 :
    ptsto γ k mq1 v1 -∗ ptsto γ k mq2 v2 -∗
      ⌜v1 = v2 ∧ match mq1, mq2 with
                 | Some q1, Some q2 => ✓(q1+q2 : fracR SI)%Qp
                 | None, None => True
                 | _, _ => False
                 end⌝.
  Proof.
    ...
    - rewrite Cinl_op pair_op frac_op'.
      ...
      apply pair_valid in Hvalid as [? Hequiv%agree_op_invL'].
      simpl in *.
      auto.
    ...
    - rewrite Cinr_op.
      ...
      apply agree_op_invL' in Hvalid. auto.
  Qed.

  Lemma ptsto_mut_agree_frac_value γ k q1 q2 v1 v2 :
    ptsto_mut γ k q1 v1 -∗ ptsto_mut γ k q2 v2 -∗ ⌜v1 = v2 ∧ ✓(q1+q2 : fracR SI)%Qp⌝.
  Proof.
    iIntros "H1 H2".
  ...
  Proof.
    ...
    { eapply auth_update_alloc,
        (alloc_singleton_local_update _ _ (Cinl (1%Qp, to_agree (v:leibnizO SI V))))=> //.
  ...
  Lemma Cinl_included_inv (A B: cmraT SI) (x:A) (y:csumR SI A B) :
    Cinl x ≼ y →
  ...
  Lemma Cinr_included_inv (A B: cmraT SI) (x:B) (y:csumR SI A B) :
    Cinr x ≼ y →
  ...
  Lemma Some_included_inv (A: cmraT SI) (x y:A) :
  ...
  Lemma Some_Cinl_included (A B: cmraT SI) (x:A) (y: csumR SI A B) :
  ...
  Lemma Some_Cinr_included (A B: cmraT SI) (x:B) (y: csumR SI A B) :
  ...
  Proof.
    ...
    there *)
    rewrite singleton_included lookup_fmap.
    ...
    - apply prod_included in Hincl as [_ Hincl]; simpl in Hincl.
  ...
  Proof.
    ...
    - iPureIntro. apply map_subseteq_spec. intros ?? Hin.
      rewrite lookup_empty in Hin. congruence.
  ...
  Proof.
    ...
    { eapply auth_update, singleton_local_update,
        (exclusive_local_update _ (Cinl (1%Qp, to_agree (v2: leibnizO SI V))))=> //.
  ...
  Proof.
    ...
    - rewrite dom_insert_L in H1.
      ...
      intros.
      assert (d1 = {[a]} ∪ (d1 ∖ {[a]})) as Hsplit.
      { apply set_split_element. set_solver. }
      rewrite Hsplit in H1.
      rewrite -assoc_L in H1.
      apply (not_elem_of_dom (D := gset K)) in H. 
  ...
  Lemma dom_singleton_inv (m: gmap K V) (a : K) :
    dom (gset K) m = {[a]} →
    ∃ (v : V), m = {[a := v]}.
  Proof.
    intros.
    ...
    - rewrite lookup_insert_ne; eauto.
      ...
      apply (not_elem_of_dom (D := gset K)). set_solver.
  Qed.
  ...
  Proof.
    ...
    - rewrite big_sepM_insert //.
      ...
      iFrame.
      assert (m2 ∪ <[l := v']> m =({[l := v']} ∪ m2 ∪ m)) as ->; eauto.
  ...
  Proof.
    ...
    iMod (own_update_2 with "Hm Hk") as "[Hm $]".
    { eapply auth_update, singleton_local_update,
        (exclusive_local_update _ (Cinr (to_agree (v: leibnizO SI V))))=> //.
      rewrite lookup_fmap Hlookup //=. }
    iModIntro.
